...
/* storage */
.storage-row{display:grid;grid-template-columns:56px 1fr 100px 90px 90px 150px;gap:0;align-items:center;border-bottom:1px solid var(--border);background:var(--bg2);transition:background .15s}
...
@media (max-width: 720px) {
  ...
  .storage-row{grid-template-columns:auto 1fr;gap:6px 10px;padding:10px 8px}
  .sr-cell{padding:4px 0}
  .sr-cell-move{grid-column:1;grid-row:1}
  .sr-cell-name{grid-column:2;grid-row:1}
...
<div class="pane active" id="pane-suppliers">
  <div class="card">
    ...
    <div class="g3">
      <div style="grid-column:1/3"><label>Company name</label><input id="s-name" placeholder="e.g. Metro Cash & Carry"></div>
...
<div class="pane" id="pane-storage">
  <div class="card" style="padding:0;overflow:hidden">
    <div class="storage-header-row" style="display:grid;grid-template-columns:56px 1fr 100px 90px 90px 150px;gap:0;border-bottom:1px solid var(--border);padding:8px 10px;background:var(--bg3)">
      <div class="card-title" style="margin:0;font-size:10px">Priority</div>
      <div class="card-title" style="margin:0;font-size:10px">Product</div>
...
function go(tab,btn) {
  ...
  ({suppliers:renderSuppliers,delivery:initDeliveryTab,storage:()=>{syncStorage();renderStorage();},recipes:renderRecipes,today:initTodayTab,price:renderPriceCalc,expiration:renderExpiration})[tab]?.();
}
...
function addSupplier() {
  ...
  suppliers.push({name,products:[]}); persist();
...
function saveDelivery() {
  ...
  if (editingDelivery>=0) {
    ...
    editingDelivery=-1;
    document.getElementById('save-del-btn').textContent='<span class="ic"><svg width="12" height="12" viewBox="0 0 12 12"><path d="M2 6.5l3 3 5-6" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg></span> Save delivery & update storage';
  } else {
...
function editDelivery(i) {
  ...
  updateDeliveryTotals();
  document.getElementById('save-del-btn').textContent='<span class="ic"><svg width="12" height="12" viewBox="0 0 12 12"><path d="M2 6.5l3 3 5-6" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg></span> Update delivery';
  // scroll to form
...
/* ══ STORAGE ══ */
function moveStorageItem(idx,dir){
  const ni=idx+dir; if(ni<0||ni>=storage.length)return;
  [storage[idx],storage[ni]]=[storage[ni],storage[idx]]; persist(); renderStorage();
}
function togglePriority(idx){storage[idx].priority=!storage[idx].priority;persist();renderStorage();}
function updateStorageQty(idx,val){storage[idx].qty=parseFloat(val)||0;persist();}
function updateStorageMin(idx,val){storage[idx].minQty=parseFloat(val)||0;persist();}
function renderStorage(){
  const list=document.getElementById('storage-list');
  if(!storage.length){list.innerHTML='<div class="empty">No products yet — add them in Suppliers.</div>';return;}
  list.innerHTML=storage.map((item,i)=>`
    <div class="${item.priority?'storage-row priority':'storage-row'}">
      <div class="sr-cell sr-cell-move">
        <div class="sr-move">
          <button class="icon-btn move" onclick="moveStorageItem(${i},-1)" style="padding:1px 5px;font-size:11px"><span class="ic"><svg width="9" height="9" viewBox="0 0 10 10"><polygon points="5,1 9,8 1,8" fill="currentColor"/></svg></span></button>
          <button class="icon-btn move" onclick="moveStorageItem(${i},1)"  style="padding:1px 5px;font-size:11px"><span class="ic"><svg width="9" height="9" viewBox="0 0 10 10"><polygon points="1,2 9,2 5,9" fill="currentColor"/></svg></span></button>
        </div>
      </div>
      <div class="sr-cell sr-cell-name" style="display:flex;align-items:center;gap:8px">
        <button class="icon-btn" onclick="togglePriority(${i})" style="font-size:18px;padding:0 2px">
      ...
      <div class="sr-cell sr-cell-avail">
        ...
        <span class="sr-control">
          <input type="number" class="qty-inp-storage" value="${item.qty}" min="0" step="0.01"
            oninput="updateStorageQty(${i},this.value)" onblur="renderStorage()">
      ...
      <div class="sr-cell sr-cell-min">
        ...
        <span class="sr-control">
          <input type="number" class="min-inp" value="${item.minQty}" min="0" step="0.01"
            oninput="updateStorageMin(${i},this.value)" onblur="renderStorage()">
          <span class="sr-unit">${item.unit}</span>
        </span>
      </div>
...
function editRecipe(i){
  ...
  document.getElementById('recipe-form-label').textContent='<span class="ic"><svg width="12" height="12" viewBox="0 0 12 12"><path d="M8.5 1.5l2 2L4 10l-2.5.5L2 8z" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linejoin="round"/></svg></span> Editing: '+r.name;
...
// Convert a quantity from recipeUnit to storageUnit for stock calculations
...
// Convert qty from recipeUnit to deliveryUnit so we can calculate cost correctly
// e.g. recipe needs 50g, delivery unit is kg <span class="ic"><svg width="11" height="11" viewBox="0 0 12 12"><line x1="2" y1="6" x2="10" y2="6" stroke="currentColor" stroke-width="1.8"/><path d="M7 2l3 4-3 4" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg></span> convert 50g to 0.05kg
function convertToDeliveryUnit(qty, recipeUnit, deliveryUnit) {
  if (recipeUnit === deliveryUnit) return qty;
  // weight conversions
  if (recipeUnit === 'g'  && deliveryUnit === 'kg')  return qty / 1000;
  if (recipeUnit === 'kg' && deliveryUnit === 'g')   return qty * 1000;
  // volume conversions
  if (recipeUnit === 'ml' && deliveryUnit === 'L')   return qty / 1000;
  if (recipeUnit === 'L'  && deliveryUnit === 'ml')  return qty * 1000;
  // same family but different scale — best effort passthrough
  return qty;
}
  ...
  list.innerHTML = recipes.map((r, ri) => {
    const ings = r.ingredients.map((ing, ii) => {
      ...
      if (delPrice) {
        ...
        const recipeQtyInDelUnit = convertToDeliveryUnit(1, ing.unit, delPrice.unit);
...
function recalcPc(ri) {
  ...
  if (subEl)   subEl.textContent   = targetPortions === r.portions
    ? `${r.portions} portion${r.portions!==1?'s':''} (base)`
    : `Scaled: ${r.portions} <span class="ic"><svg width="11" height="11" viewBox="0 0 12 12"><line x1="2" y1="6" x2="10" y2="6" stroke="currentColor" stroke-width="1.8"/><path d="M7 2l3 4-3 4" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg></span> ${targetPortions} portions`;
}
