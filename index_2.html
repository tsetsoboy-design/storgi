...
.status{padding:10px 14px;border-radius:8px;font-size:13px;margin-bottom:10px}
...
<div class="card" id="login-card">
  <div class="card-title" style="font-size:11px;color:var(--text3);text-transform:uppercase;letter-spacing:.05em;margin-bottom:10px">Admin Login</div>
...
async function signUp() {
  ...
  if (error) { showStatus('login-status', 'Error: ' + error.message, 'err'); return; }
  if (data.session) {
    showStatus('login-status','Account created and signed in!','ok');
    onSignedIn(data.session);
  } else {
    showStatus('login-status','Account created. Check your email to confirm, then sign in. (If email confirmation is required — see note below.)','ok');
  }
}
...
async function addSupplier() {
  ...
  const { error } = await sb.from('suppliers').insert({ name, products: [] });
  if (error) { showStatus('db-status','Error: ' + error.message, 'err'); return; }
...
async function loadSuppliers() {
  ...
  if (error) { showStatus('db-status','Error loading: ' + error.message, 'err'); return; }
